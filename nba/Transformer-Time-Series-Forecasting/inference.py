    ...
    with torch.no_grad():
        ...
        for plot in range(25):
            ...
            for index_in, index_tar, _input, target, player in dataloader:
                ...
                val_loss += loss
                player_predictions.append(all_predictions[-forecast_window:,0,0].mean().item())
                players.append(player[0])

            
            val_loss = val_loss/10
            src_humidity = src[:,:,0].cpu()
            target_humidity = target[:,:,0].cpu()
            prediction_humidity = all_predictions[:,:,0].detach().cpu().numpy()
            plot_prediction(plot, path_to_save_predictions, src_humidity, target_humidity, prediction_humidity, player, index_in, index_tar)

            data = {'players': players, 'preds': player_predictions}
            dump = pd.DataFrame(data=data)
